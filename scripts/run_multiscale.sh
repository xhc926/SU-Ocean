#!/bin/bash
# Multiscale: 多要素 + iTransformerUniOcean，固定 ALL2（area2）。
...
for SEQ_LEN in "${SEQ_LENS[@]}"; do
  ...
  for SCALES in "${SCALE_SETS[@]}"; do
    ...
    echo "=========================================="
    python -u run.py \
      --checkpoints /root/autodl-tmp/ms/checkpoints/ \
      --log_dir /root/autodl-tmp/ms/logs \
      --results_dir /root/autodl-tmp/ms/results \
      --model $MODEL \
      --data $DATA \
