    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, 
        ...
        if self.use_multi_scale:
            ...
            if self.output_attention:
                ...
                return torch.stack((dec_out_one[:,-self.pred_len:,:],dec_out_two[:,-self.pred_len:,:],dec_out_three[:,-self.pred_len:,:]), axis=3)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, 
        ...
        for scale in scales:
            ...
            if scale>=4:
                ...
                all=torch.stack((dec_out_coarse_one,dec_out_coarse_two),axis=3)
                B, L, D, N = all.shape
                all = all.permute(0, 2, 3, 1)
                all = all.reshape(B, D * N, L)
                all = self.ffn2drop1(self.ffn2pw1(all))
