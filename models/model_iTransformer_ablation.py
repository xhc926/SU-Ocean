...
from models.model_iTransformer import iTransformerUni


class _SelfFactorSelector(nn.Module):
    """
    Select only one factor slice from concatenated [f1, f2, f3] features.
    """
...
class iTransformerUniAbl(iTransformerUni):
    """
    Ablation model:
    - Keep 3-factor inputs and per-factor encoders unchanged.
    ...
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len,
        ...
        # Force full 3-factor forward path; this class is for "keep inputs, remove cross-factor fusion".
