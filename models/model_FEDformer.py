...
from models.autoformer_encdec import Encoder, Decoder, EncoderLayer, DecoderLayer, my_Layernorm, series_decomp, series_decomp_multi,Decoder_base, DecoderLayer_base
from models.attn import FourierBlock, FourierCrossAttention, AutoCorrelationLayer
from models.attn import MultiWaveletCross, MultiWaveletTransform
...
class FEDformerUni(nn.Module):
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len, 
        ...
        super(FEDformerUni, self).__init__()
        ...
        if version == 'Wavelets':
            ...
            decoder_cross_att = MultiWaveletCross(in_channels=d_model, out_channels=d_model,
                                                  seq_len_q=self.seq_len // 2 + self.pred_len,
                                                  seq_len_kv=self.seq_len, modes=modes, ich=d_model,
                                                  base=base, activation=cross_activation)
        else:
            encoder_self_att = FourierBlock(in_channels=d_model, out_channels=d_model,
                                            seq_len=self.seq_len, modes=modes, mode_select_method=mode_select)
            decoder_self_att = FourierBlock(in_channels=d_model, out_channels=d_model,
                                            seq_len=self.seq_len//2+self.pred_len, modes=modes, mode_select_method=mode_select)
            decoder_cross_att = FourierCrossAttention(in_channels=d_model, out_channels=d_model,
                                                      seq_len_q=self.seq_len//2+self.pred_len,
                                                      seq_len_kv=self.seq_len, modes=modes, mode_select_method=mode_select)
        enc_modes = int(min(modes, seq_len//2))
        dec_modes = int(min(modes, (seq_len//2+out_len)//2))
        print('enc_modes: {}, dec_modes: {}'.format(enc_modes, dec_modes))

        self.encoder = Encoder(
            [EncoderLayer(AutoCorrelationLayer(encoder_self_att, d_model, n_heads),
                    d_model, d_ff, moving_avg=move_avg, dropout=dropout, activation=activation
        ...
        self.decoder = Decoder(
            [DecoderLayer(AutoCorrelationLayer(decoder_self_att, d_model, n_heads),
                    ...
                    d_model, c_out, d_ff, moving_avg=move_avg, dropout=dropout, activation=activation,
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        for scale in scales:
            ...
            if scale>=4:
                ...
                dec_out_coarse_one=seasonal_part_one
            else:
            ...
            dec_out_coarse_two=seasonal_part_two
            ...
            if scale>=4:
                ...
                dec_out_coarse_three=seasonal_part_three
            else:
            ...
            if scale>=4:
                ...
                all=torch.stack((dec_out_coarse_one,dec_out_coarse_two,dec_out_coarse_three),axis=3)
                B, L, D, N = all.shape
                all = all.permute(0, 2, 3, 1)
...
class FEDformer(nn.Module):
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len, 
        ...
        self.decoder = Decoder_base(
            [DecoderLayer_base(AutoCorrelationLayer(decoder_self_att, d_model, n_heads),
                    AutoCorrelationLayer(decoder_cross_att, d_model, n_heads),
                    d_model, c_out, d_ff, moving_avg=move_avg, dropout=dropout, activation=activation,
                ) for l in range(d_layers)],
            norm_layer=my_Layernorm(d_model),
