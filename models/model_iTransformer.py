...
class iTransformerUni5(nn.Module):
    """
    iTransformer for multi-factor prediction (hardcoded 5 factors).
    Coarse-to-Fine hierarchical residual across spatial scales.
    """
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len,
                factor=5, d_model=512, n_heads=8, e_layers=3, d_layers=2, d_ff=512, move_avg=25,
                ...
                land_mask_path='',
                scale_mask_mode='soft'):
        super(iTransformerUni5, self).__init__()
        self.pred_len = out_len
        self.attn = attn
        self.output_attention = output_attention
        self.seq_len = seq_len
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if x_enc.shape[-1] < self.factor_num:
            raise ValueError("iTransformerUni5 expects 5 factors in x_enc, got {}".format(x_enc.shape[-1]))
